...
- DEDUPE: one row per recording (norm title + year), preferring the clean
  cbcoh copy over CBC-top / Be-A-Berean(old) twins.
    ...
    python tools/import_catalog.py --wipe          # build db/import.sql
    python tools/import_catalog.py --wipe --apply  # build + apply to local D1

*** THIS TOOL IS A FULL RESEED AND IS NO LONGER THE NORMAL PATH. ***

It emits `DELETE FROM items;` and rebuilds the catalog from a folder/feed scan.
The live catalog now holds work that NO SCAN CAN REPRODUCE:
...
tools instead, which touch one row and nothing else:

    tools/add_sermon.py        add one new recording
    tools/attach_transcript.py (re)index one transcript
    tools/attach_extras.py     video link + downloadable files
"""
...
def main() -> None:
    # A full reseed destroys hand-curated data (see the module docstring), so it
    # has to be asked for explicitly. Refusing here — before anything is written
    # — also stops the run from regenerating the delete_*.bat source-audio
    # deleters as a side effect.
    if "--wipe" not in sys.argv:
        sys.exit(
            "import_catalog.py performs a FULL RESEED (DELETE FROM items) and would\n"
            "destroy hand-corrected passages, titles, video links and attachments\n"
            "(item_files cascades). It is not the normal path any more.\n\n"
            "  Add one recording ....... python tools/add_sermon.py\n"
            "  Re-index a transcript ... python tools/attach_transcript.py --slug SLUG\n"
            "  Video link / handouts ... python tools/attach_extras.py --slug SLUG\n\n"
            "If you really do want to rebuild the whole catalog from scratch, pass --wipe."
        )
    ...
    lines = ["-- FULL RESEED — DESTRUCTIVE. Generated by import_catalog.py --wipe.",
             "-- Applying this DROPS every hand-made edit: corrected passages and titles,",
             "-- items.video_url, and all item_files rows (they cascade with the item).",
             "-- Do not apply to production unless you mean to rebuild from the folder scan.",
             "DELETE FROM scripture_refs;", "DELETE FROM item_transcripts;",
             "DELETE FROM item_fts;", "DELETE FROM item_files;", "DELETE FROM items;",
